...
function Wait-ADReady {
    param([string]$VMName, [int]$MaxAttempts = 10, [int]$DelaySeconds = 30)
    Write-Host "  Waiting for Active Directory services to stabilize on $VMName..." -ForegroundColor Yellow
    $checkScript = 'try { Import-Module ActiveDirectory -ErrorAction Stop; Get-ADDomain -ErrorAction Stop | Out-Null; Write-Output "AD_READY" } catch { Write-Output "AD_NOT_READY" }'
    $tempFile = [System.IO.Path]::GetTempPath() + [System.IO.Path]::GetRandomFileName() + ".ps1"
    $checkScript | Out-File -FilePath $tempFile -Encoding UTF8
    for ($i = 1; $i -le $MaxAttempts; $i++) {
        $jsonLines = az vm run-command invoke --resource-group $ResourceGroup --name $VMName --command-id RunPowerShellScript --scripts "@$tempFile" --output json --only-show-errors 2>$null
        $result = ($jsonLines -join "`n") | ConvertFrom-Json
        $stdout = ($result.value | Where-Object { $_.code -like "*StdOut*" }).message
        if ($stdout -match "AD_READY") {
            Write-Host "  Active Directory is ready (attempt $i)." -ForegroundColor Green
            Remove-Item $tempFile -ErrorAction SilentlyContinue
            return
        }
        Write-Host "    Attempt $i/$MaxAttempts -- AD not ready yet, waiting ${DelaySeconds}s..." -ForegroundColor DarkGray
        Start-Sleep -Seconds $DelaySeconds
    }
    Remove-Item $tempFile -ErrorAction SilentlyContinue
    Write-Warning "  AD did not report ready after $($MaxAttempts * $DelaySeconds)s. Proceeding anyway."
}
...
Start-Sleep -Seconds 60; Wait-VMOnline -VMName "DC01"; Wait-ADReady -VMName "DC01"
...
Write-Host "RDP into CLIENT01 as: LAB\joe.meyer  Password: P@ssw0rd123!"